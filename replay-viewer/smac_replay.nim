import
  std/json,
  smac/[broadcast, global, replay_runtime, replays, sim]
...
## used to die with smac_error_len() == 0 and no diagnostic at all). The
...
proc smacLoadReplay(data: ptr uint8, length: cint): cint
    {.exportc: "smac_load_replay", cdecl.} =
  try:
    lastError = ""
...
proc smacInput(data: ptr uint8, length: cint)
    {.exportc: "smac_input", cdecl.} =
  if runtimeLoaded:
    viewer.applyGlobalViewerMessage(data.bytesFromPointer(int(length)))

proc smacFrame(): cint {.exportc: "smac_frame", cdecl.} =
...
proc smacPacketPointer(): ptr uint8
    {.exportc: "smac_packet_ptr", cdecl.} =
  if packet.len == 0:
    nil
  else:
    packet[0].addr

proc smacPacketLength(): cint {.exportc: "smac_packet_len", cdecl.} =
  cint(packet.len)
...
proc smacErrorLength(): cint {.exportc: "smac_error_len", cdecl.} =
  cint(lastError.len)

proc smacStagePointer(): ptr uint8 {.exportc: "smac_stage_ptr", cdecl.} =
  ## The progress note (see stageNote above). Unlike smac_error_*, this stays
...
proc smacStageLength(): cint {.exportc: "smac_stage_len", cdecl.} =
  cint(stageNoteLen)
...
when isMainModule and defined(emscripten):
  ...
  # module stays alive and JS keeps calling smac_load_replay/smac_frame. The
