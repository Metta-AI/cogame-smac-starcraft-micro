...
# use-after-free fixed in smac_replay.nim (emscripten_exit_with_live_runtime)
...
# smac_error_len(). Aborting keeps linear memory intact, and the page reads
# smac_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "smac_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_smac_load_replay,_smac_frame,_smac_input,_smac_packet_ptr,_smac_packet_len,_smac_mismatch_tick,_smac_replay_tick,_smac_replay_max_tick,_smac_error_ptr,_smac_error_len,_smac_stage_ptr,_smac_stage_len
  """).replace("\n", " ")
)
