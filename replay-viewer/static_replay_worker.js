...
function stageNote() {
  ...
  try {
    var length = Module._smac_stage_len ? Module._smac_stage_len() : 0;
    if (!length) return '';
    var pointer = Module._smac_stage_ptr();
...
function runtimeError() {
  var length = Module._smac_error_len();
  if (!length) {
    var stage = stageNote();
    return stage
  ...
  var pointer = Module._smac_error_ptr();
  return new TextDecoder().decode(
    Module.HEAPU8.slice(pointer, pointer + length));
}
...
function sendRuntimeInput(bytes) {
  ...
  copyIntoRuntime(bytes, function (pointer, length) {
    Module._smac_input(pointer, length);
  });
}
...
async function start() {
  ...
  try {
    ...
    var loaded = copyIntoRuntime(bytes, function (pointer, length) {
      return Module._smac_load_replay(pointer, length);
    });
    ...
    postMessage({
      type: 'loaded',
      mismatchTick: Module._smac_mismatch_tick()
    });
  } catch (error) {
    reportFailure(error);
  }
...
function applyInputNow() {
  ...
  try {
    if (Module._smac_frame() < 0) throw new Error(runtimeError());
    ingestPacket();
    postMessage({
      type: 'inputApplied',
      mismatchTick: Module._smac_mismatch_tick()
...
importScripts('./wire_constants.js', './broadcast_core.js', './smac_replay.js');
