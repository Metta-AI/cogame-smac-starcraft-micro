...
proc defaultGameConfig*(): GameConfig =
  ...
  GameConfig(
    ...
    roles: @[],
    enemyRoles: @[],
    friendlySpawnX: DefaultFriendlySpawnX,
    enemySpawnX: DefaultEnemySpawnX,
    spawnSpacingPx: DefaultSpawnSpacingPx,
    spawnJitterPx: DefaultSpawnJitterPx,
    rangerHp: DefaultRangerHp,
    rangerRange: DefaultRangerRange,
    rangerDamage: DefaultRangerDamage,
    rangerCooldown: DefaultRangerCooldown,
    bladeHp: DefaultBladeHp,
    bladeSpeedPct: DefaultBladeSpeedPct,
    bladeReach: DefaultBladeReach,
    bladeArcBrads: DefaultBladeArcBrads,
    bladeDamage: DefaultBladeDamage,
    bladeCooldown: DefaultBladeCooldown,
    swingTicks: DefaultSwingTicks,
    swarmHp: DefaultSwarmHp,
    swarmSpeedPct: DefaultSwarmSpeedPct,
    swarmReach: DefaultSwarmReach,
    swarmDamage: DefaultSwarmDamage,
    swarmCooldown: DefaultSwarmCooldown,
    aggroPx: DefaultAggroPx,
    leashPx: DefaultLeashPx,
    retargetTicks: DefaultRetargetTicks,
    enemyStuckTicks: DefaultEnemyStuckTicks,
    rangerStandoff: DefaultRangerStandoff,
    kiteStandoff: DefaultKiteStandoff,
    screenStandoff: DefaultScreenStandoff,
    chaseCapPx: DefaultChaseCapPx,
    panicPx: DefaultPanicPx,
    winWeightPermille: DefaultWinWeightPermille,
    dmgWeightPermille: DefaultDmgWeightPermille,
    survWeightPermille: DefaultSurvWeightPermille,
    creditEpsilonPerMyriad: DefaultCreditEpsilonPerMyriad
  )
...
proc readConfigInt(node: JsonNode, name: string, value: var int) =
  ...
  let item = node[name]
  if item.kind != JInt:
    raise newException(SmacError, "Config field " & name & " must be an integer.")
  value = item.getInt()
...
proc readConfigBool(node: JsonNode, name: string, value: var bool) =
  ...
  let item = node[name]
  if item.kind != JBool:
    raise newException(SmacError, "Config field " & name & " must be a boolean.")
  value = item.getBool()
...
proc readConfigString(node: JsonNode, name: string, value: var string) =
  ...
  if item.kind != JString:
    raise newException(SmacError, "Config field " & name & " must be a string.")
  value = item.getStr()
...
proc readConfigSlots(node: JsonNode, slots: var seq[PlayerSlotConfig]) =
  ...
  if items.kind != JArray:
    raise newException(SmacError, "Config field slots must be an array.")
  slots.setLen(0)
...
proc readConfigPlayers(node: JsonNode, slots: var seq[PlayerSlotConfig]) =
  ...
  if items.kind != JArray:
    raise newException(SmacError, "Config field players must be an array.")
...
proc readConfigTokens(
  ...
  if items.kind != JArray:
    raise newException(SmacError, "Config field tokens must be an array.")
  if items.len > MaxPlayers:
    raise newException(
      SmacError,
...
proc readConfigPerks(node: JsonNode, config: var GameConfig) =
  ...
  if perks.kind != JObject:
    raise newException(SmacError, "Config field perks must be an object.")
...
proc readConfigPerkMods(node: JsonNode, config: var GameConfig) =
  ...
  if mods.kind != JObject:
    raise newException(SmacError, "Config field perkMods must be an object.")
  for key in mods.keys:
    if key notin ["armorHp", "scopeAim", "grenadeRange", "thrusterSpeed",
        "luckChance", "luckDamage"]:
      raise newException(
        SmacError, "Config field perkMods has unknown key " & key & ".")
  ...
  if config.perkMods.luckDamage < 1:
    raise newException(
      SmacError, "Config field perkMods.luckDamage must be at least 1.")
...
proc readConfigHandicaps(node: JsonNode, config: var GameConfig) =
  ...
  if handicaps.kind != JObject:
    raise newException(SmacError, "Config field handicaps must be an object.")
...
proc readConfigRegimes(node: JsonNode, config: var GameConfig) =
  ...
  if items.kind != JArray:
    raise newException(SmacError, "Config field regimes must be an array.")
  if items.len < 1 or items.len > 4:
    raise newException(
      SmacError, "Config field regimes must have 1..4 entries.")
...
proc readConfigRoles(
  node: JsonNode, name: string, roles: var seq[UnitRole]
) =
  ## Reads a role array ("roles" / "enemyRoles"). Bounded at MaxEnemyUnits so
  ## no variant can schedule an unbounded army.
  if not node.hasKey(name):
    return
  let items = node[name]
  if items.kind != JArray:
    raise newException(SmacError, "Config field " & name & " must be an array.")
  if items.len < 1 or items.len > MaxEnemyUnits:
    raise newException(
      SmacError,
      "Config field " & name & " must have 1.." & $MaxEnemyUnits & " entries."
    )
  var parsed: seq[UnitRole] = @[]
  for i, item in items.elems:
    if item.kind != JString:
      raise newException(
        SmacError, "Config field " & name & "[" & $i & "] must be a string.")
    parsed.add(parseUnitRole(item.getStr()))
  roles = parsed

proc applyMicroDefaults*(config: var GameConfig) =
  ## Derives the inherited combat-core knobs the micro loadout drives from its
  ## own role table, so the starter's hitscan gun IS the ranger's shot and the
  ## starter's arc cone IS the blade's swing with no second source of truth.
  ## A config that is not micro is untouched.
  if config.loadout != LoadoutMicro:
    return
  config.gunRange = max(1, config.rangerRange)
  config.fireCooldownTicks = max(1, config.rangerCooldown)
  config.hitPoints = max(1, config.rangerHp)
  config.lives = 1
  config.respawnTicks = 0
  config.cogsPerTeam = 1
  config.teams = 2
  config.floorPaint = false
  config.paintBuff = false
  config.hill = false
  config.barrierPickups = 0
  config.barrageMaxPerSec = 0
  for team in Team:
    config.handicaps[team] = 0
  if config.roles.len == 0:
    for _ in 0 ..< max(1, config.numAgents):
      config.roles.add(urRanger)
  if config.enemyRoles.len == 0:
    config.enemyRoles = config.roles

proc validateMicro(config: GameConfig) =
  ## The micro loadout's own bounds. Everything here is a rule a variant could
  ## otherwise break silently, and each is asserted in tests/test_manifest.nim.
  if config.numAgents < 1:
    raise newException(
      SmacError, "Micro config must set num_agents (one seat per unit).")
  if config.roles.len != config.numAgents:
    raise newException(
      SmacError,
      "Micro config field roles must have exactly num_agents (" &
        $config.numAgents & ") entries; got " & $config.roles.len & "."
    )
  for role in config.roles:
    if role == urSwarm:
      raise newException(
        SmacError, "Micro config field roles may not contain 'swarm'.")
  if config.enemyRoles.len < 1 or config.enemyRoles.len > MaxEnemyUnits:
    raise newException(
      SmacError,
      "Micro config field enemyRoles must have 1.." & $MaxEnemyUnits &
        " entries."
    )
  if config.maxGames < 1:
    raise newException(SmacError, "Micro config field maxGames must be >= 1.")
  if config.maxTicks < 1:
    raise newException(SmacError, "Micro config field maxTicks must be >= 1.")
  if config.spawnJitterPx < 0:
    raise newException(
      SmacError, "Micro config field spawnJitterPx must not be negative.")
  if config.spawnSpacingPx < 1:
    raise newException(
      SmacError, "Micro config field spawnSpacingPx must be >= 1.")
  if config.bladeArcBrads < 1 or config.bladeArcBrads > 128:
    raise newException(
      SmacError, "Micro config field bladeArcBrads must be 1..128.")
  if config.swingTicks < 1:
    raise newException(SmacError, "Micro config field swingTicks must be >= 1.")
  if config.aggroPx < 1 or config.leashPx < config.aggroPx:
    raise newException(
      SmacError, "Micro config needs 0 < aggroPx <= leashPx.")
  if config.retargetTicks < 1:
    raise newException(
      SmacError, "Micro config field retargetTicks must be >= 1.")
  if config.enemyStuckTicks < 1:
    raise newException(
      SmacError, "Micro config field enemyStuckTicks must be >= 1.")
  if config.winWeightPermille + config.dmgWeightPermille +
      config.survWeightPermille != 1000:
    raise newException(
      SmacError,
      "Micro scoring weights must sum to 1000 permille; got " &
        $(config.winWeightPermille + config.dmgWeightPermille +
          config.survWeightPermille) & "."
    )
  if config.winWeightPermille < 0 or config.dmgWeightPermille < 0 or
      config.survWeightPermille < 0:
    raise newException(
      SmacError, "Micro scoring weights must not be negative.")
  if config.creditEpsilonPerMyriad < 0 or config.creditEpsilonPerMyriad > 100:
    raise newException(
      SmacError, "Micro config field creditEpsilonPerMyriad must be 0..100.")
  if config.wallClockBudgetSeconds < 1 or config.wallClockBudgetSeconds > 720:
    raise newException(
      SmacError,
      "Micro config field wallClockBudgetSeconds must be 1..720 (60% of the " &
        "assumed 1200 s episode timeout)."
    )

proc validate(config: GameConfig) =
  ## Raises if a gameplay config has invalid values.
  if config.motionScale <= 0:
    raise newException(SmacError, "Config field motionScale must be positive.")
  if config.frictionDen <= 0:
    raise newException(SmacError, "Config field frictionDen must be positive.")
  if config.minPlayers < 1:
    raise newException(SmacError, "Config field minPlayers must be at least 1.")
  if config.teams notin [2, 4]:
    raise newException(SmacError, "Config field teams must be 2 or 4.")
  ...
  if config.minPlayers > MaxPlayers:
    raise newException(SmacError, "can't do more than 8 players.")
  if config.lives < 1:
    raise newException(SmacError, "Config field lives must be at least 1.")
  if config.hitPoints < 1:
    raise newException(SmacError, "Config field hitPoints must be at least 1.")
  if config.gunRange <= 0:
    raise newException(SmacError, "Config field gunRange must be positive.")
  if config.fireWindupTicks < 0:
    raise newException(SmacError, "Config field fireWindupTicks must not be negative.")
  if config.carrierSpeedPct <= 0 or config.carrierSpeedPct > 100:
    raise newException(SmacError, "Config field carrierSpeedPct must be 1..100.")
  if config.playerBouncePct < 0 or config.playerBouncePct > 100:
    raise newException(SmacError, "Config field playerBouncePct must be 0..100.")
  if config.aimTurnRate < 1:
    raise newException(SmacError, "Config field aimTurnRate must be at least 1.")
  if config.visionConeDeg < 0 or config.visionConeDeg > 180:
    raise newException(SmacError, "Config field visionConeDeg must be between 0 and 180.")
  if config.puddleDamagePct < 0 or config.puddleDamagePct > 100:
    raise newException(SmacError, "Config field puddleDamagePct must be 0..100.")
  if config.barrierPickups < 0 or
      config.barrierPickups > MaxBarrierPickupsPerTeam:
    raise newException(SmacError,
      "Config field barrierPickups must be 0.." & $MaxBarrierPickupsPerTeam & ".")
  if config.visionBubble < 0:
    raise newException(SmacError, "Config field visionBubble must be non-negative.")
  ...
  if config.startWaitTicks < 0:
    raise newException(SmacError, "Config field startWaitTicks must be non-negative.")
  if config.lobbyJoinTimeoutTicks < 0:
    raise newException(SmacError, "Config field lobbyJoinTimeoutTicks must be non-negative.")
  if config.respawnTicks < 0 or config.fireCooldownTicks < 0:
    raise newException(SmacError, "Timer config fields must not be negative.")
  ...
  if config.barrageMaxPerSec > 0:
    ...
    if config.barrageStartSec < 1:
      raise newException(
        SmacError, "Config field barrageStartSec must be at least 1.")
    if config.barrageSaturateSec < 1:
      raise newException(
        SmacError, "Config field barrageSaturateSec must be at least 1.")
  if config.loadout notin [LoadoutClassic, LoadoutPaint, LoadoutMicro]:
    raise newException(
      SmacError,
      "Config field loadout must be " & LoadoutClassic & ", " &
        LoadoutPaint & " or " & LoadoutMicro & "; got " & config.loadout & "."
    )
  if config.loadout == LoadoutMicro:
    config.validateMicro()
  if config.cogsPerTeam < 1 or config.cogsPerTeam > 8:
    raise newException(SmacError, "Config field cogsPerTeam must be 1..8.")
  if config.paintTile < 4:
    raise newException(SmacError, "Config field paintTile must be at least 4.")
  if config.hillRadiusTiles < 0:
    raise newException(
      SmacError, "Config field hillRadiusTiles must not be negative.")
  ...
  if config.hillDecisiveTicks < 1:
    raise newException(
      SmacError, "Config field hillDecisiveTicks must be at least 1.")
  if config.paintSpeedOwnPct < 1 or config.paintSpeedEnemyPct < 1:
    raise newException(
      SmacError, "Config field paintSpeed*Pct must be positive.")
  if config.paintHealTicks < 1:
    raise newException(
      SmacError, "Config field paintHealTicks must be at least 1.")
  if config.sprayDamage < 1:
    raise newException(
      SmacError, "Config field sprayDamage must be at least 1.")
  if config.regimes.len < 1 or config.regimes.len > 4:
    raise newException(
      SmacError, "Config field regimes must have 1..4 entries.")
  if config.turnTicks < 1:
    raise newException(SmacError, "Config field turnTicks must be at least 1.")
  if config.turnBudgetMs < 1 or config.attempt1Ms < 1 or config.retryMs < 1:
    raise newException(
      SmacError, "Config LLM deadline fields must be positive.")
  ...
  if config.turnSpacingMs < 0:
    raise newException(
      SmacError, "Config field turnSpacingMs must not be negative.")
  if config.wallClockBudgetSeconds < 1:
    raise newException(
      SmacError, "Config field wallClockBudgetSeconds must be positive.")
  if config.paintBuff and not config.floorPaint:
    raise newException(
      SmacError, "Config field paintBuff requires floorPaint.")
  if config.hill and not config.floorPaint:
    raise newException(
      SmacError, "Config field hill requires floorPaint.")
  if config.numAgents < 0:
    raise newException(SmacError, "Config field num_agents must not be negative.")
  if config.slots.len > MaxPlayers:
    raise newException(SmacError, "Config field slots cannot have more than 8 entries.")
...
proc update*(config: var GameConfig, jsonText: string) =
  ...
  try:
    ...
    raise newException(SmacError, "Could not parse config JSON: " & e.msg)
  if node.kind != JObject:
    raise newException(SmacError, "Config must be a JSON object.")
  ...
  node.readConfigString("scenario", config.scenario)
  node.readConfigBool("fogOfWar", config.fogOfWar)
  node.readConfigRoles("roles", config.roles)
  node.readConfigRoles("enemyRoles", config.enemyRoles)
  node.readConfigInt("friendlySpawnX", config.friendlySpawnX)
  node.readConfigInt("enemySpawnX", config.enemySpawnX)
  node.readConfigInt("spawnSpacingPx", config.spawnSpacingPx)
  node.readConfigInt("spawnJitterPx", config.spawnJitterPx)
  node.readConfigInt("rangerHp", config.rangerHp)
  node.readConfigInt("rangerRange", config.rangerRange)
  node.readConfigInt("rangerDamage", config.rangerDamage)
  node.readConfigInt("rangerCooldown", config.rangerCooldown)
  node.readConfigInt("bladeHp", config.bladeHp)
  node.readConfigInt("bladeSpeedPct", config.bladeSpeedPct)
  node.readConfigInt("bladeReach", config.bladeReach)
  node.readConfigInt("bladeArcBrads", config.bladeArcBrads)
  node.readConfigInt("bladeDamage", config.bladeDamage)
  node.readConfigInt("bladeCooldown", config.bladeCooldown)
  node.readConfigInt("swingTicks", config.swingTicks)
  node.readConfigInt("swarmHp", config.swarmHp)
  node.readConfigInt("swarmSpeedPct", config.swarmSpeedPct)
  node.readConfigInt("swarmReach", config.swarmReach)
  node.readConfigInt("swarmDamage", config.swarmDamage)
  node.readConfigInt("swarmCooldown", config.swarmCooldown)
  node.readConfigInt("aggroPx", config.aggroPx)
  node.readConfigInt("leashPx", config.leashPx)
  node.readConfigInt("retargetTicks", config.retargetTicks)
  node.readConfigInt("enemyStuckTicks", config.enemyStuckTicks)
  node.readConfigInt("rangerStandoff", config.rangerStandoff)
  node.readConfigInt("kiteStandoff", config.kiteStandoff)
  node.readConfigInt("screenStandoff", config.screenStandoff)
  node.readConfigInt("chaseCapPx", config.chaseCapPx)
  node.readConfigInt("panicPx", config.panicPx)
  node.readConfigInt("winWeightPermille", config.winWeightPermille)
  ...
  if node.hasKey("mapSpec"):
    if node["mapSpec"].kind != JObject:
      raise newException(SmacError, "Config field mapSpec must be an object.")
  ...
  ## map-wide default.
  let mapMeta = resolveSmacMapMetadata(config)
...
proc configJson*(config: GameConfig): string =
  ...
  # Echo the micro keys only when the mode is engaged, so a classic
  # game's replay config stays byte-identical to pre-micro builds. When
  ...
  # different game.
  let paintModeOn = config.loadout != LoadoutClassic or config.floorPaint or
  ...
  # The micro loadout echoes EVERY tuning key: the wasm viewer re-derives the
  # whole battle — spawns, the enemy army, every weapon resolution — from this
  # config, so a missing key would re-simulate a different game.
  if config.loadout == LoadoutMicro:
    ...
    node["roles"] = (block:
      var arr = newJArray()
      for role in config.roles:
        arr.add(%roleText(role))
      arr)
    node["enemyRoles"] = (block:
      var arr = newJArray()
      for role in config.enemyRoles:
        arr.add(%roleText(role))
      arr)
    node["friendlySpawnX"] = %config.friendlySpawnX
    node["enemySpawnX"] = %config.enemySpawnX
    node["spawnSpacingPx"] = %config.spawnSpacingPx
    node["spawnJitterPx"] = %config.spawnJitterPx
    node["rangerHp"] = %config.rangerHp
    node["rangerRange"] = %config.rangerRange
    node["rangerDamage"] = %config.rangerDamage
    node["rangerCooldown"] = %config.rangerCooldown
    node["bladeHp"] = %config.bladeHp
    node["bladeSpeedPct"] = %config.bladeSpeedPct
    node["bladeReach"] = %config.bladeReach
    node["bladeArcBrads"] = %config.bladeArcBrads
    node["bladeDamage"] = %config.bladeDamage
    node["bladeCooldown"] = %config.bladeCooldown
    node["swingTicks"] = %config.swingTicks
    node["swarmHp"] = %config.swarmHp
    node["swarmSpeedPct"] = %config.swarmSpeedPct
    node["swarmReach"] = %config.swarmReach
    node["swarmDamage"] = %config.swarmDamage
    node["swarmCooldown"] = %config.swarmCooldown
    node["aggroPx"] = %config.aggroPx
    node["leashPx"] = %config.leashPx
    node["retargetTicks"] = %config.retargetTicks
    node["enemyStuckTicks"] = %config.enemyStuckTicks
    node["rangerStandoff"] = %config.rangerStandoff
    node["kiteStandoff"] = %config.kiteStandoff
    node["screenStandoff"] = %config.screenStandoff
    node["chaseCapPx"] = %config.chaseCapPx
    node["panicPx"] = %config.panicPx
    node["winWeightPermille"] = %config.winWeightPermille
    node["dmgWeightPermille"] = %config.dmgWeightPermille
    node["survWeightPermille"] = %config.survWeightPermille
    node["creditEpsilonPerMyriad"] = %config.creditEpsilonPerMyriad
  ...
  # its default even with the micro gates off.
  if paintModeOn or config.sprayDamage != SprayPaintDamage:
