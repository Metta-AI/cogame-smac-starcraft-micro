...
const
  ...
  SmacReplayMagic = "COWLDSMC"
  SmacReplayFormatVersion = 1'u16
  SmacReplaySpec = ReplaySpec(
    magic: SmacReplayMagic,
    formatVersion: SmacReplayFormatVersion,
    gameName: GameName,
...
proc replayDisplaySpeed*(replay: ReplayPlayer): float =
  ## Returns the speed the chrome shows: 0.5 at half speed, else the integer
  ## speed.
...
proc applyReplayEvents(replay: var ReplayPlayer, sim: var SimServer) =
  ...
  while replay.leaveIndex < replay.data.leaves.len and
    ...
    if sim.config.numAgents > 0:
      ## Micro: a leave does NOT shift the mask arrays. The cogs are fixed
  ...
  while replay.chatIndex < replay.data.chats.len and
    ...
    # Micro CONTROL records (register / directive / fallback /
    ...
    if sim.config.numAgents > 0 and
      ...
      ## The ONE exception: the wall-clock `stop` is LOAD-BEARING. It is
      ## applied here by the identical proc the live server called, so a
      ## deadline-ended replay reproduces the recorded hash at the stop tick
      ## instead of warning on it (particle-worlds 13c66d7, 2026-08-26).
      if "\"k\":\"stop\"" in chat.message:
...
proc advanceReplayScan*(replay: var ReplayPlayer, maxTicks: int) =
  ...
  while stepsLeft > 0 and scan.builder.playing and
    ...
    for event in stepBeats:
      ...
      ## MICRO: exactly the five kinds the appended client block defines CSS
      ## for — the timeline tells the battle story and nothing else.
      let scrubberBeats =
        if scan.sim.config.microMode():
          @["battlestart", "firstblood", "loss", "lastcog", "battleover"]
        elif scan.sim.config.hill:
...
proc advanceReplayPlayback*(
  ...
  # further or resumes.
  # Frame parity for the 1/2x speed: flipped once per presentation frame,
  # before any early return, so a paused or seeking frame still advances it.
