...
const
  ...
  # ...and the same cogs holding their micro marker forward at the camera.
  ...
  ShutdownGraceSeconds = 20  ## micro squad mode: /healthz + /global keep
...
proc httpHandler(request: Request) =
  if request.path == HealthPath and request.httpMethod == "GET":
    ...
    request.respond(200, headers, "SMAC server")
...
proc runServerLoop*(
  ...
  ## MICRO: `num_agents` seats drive `num_agents` units and the server fills
  ## the rest of the roster with the scripted enemy army, so the squad path is
  ## engaged whenever the micro loadout is on — `cogsPerTeam` is 1 here.
  let squadMode = not replayLoaded and config.numAgents > 0 and
    (config.microMode() or config.cogsPerTeam > 1)
  var
    engine =
  ...
  while true:
    ...
    # episodeTimeoutSeconds, so micro always settles and scores itself
    ...
    if squadMode and not deadlineHit and
      ...
      echo "wall-clock budget of ", config.wallClockBudgetSeconds,
        "s reached; settling the episode from what is banked at this tick"
      if config.microMode():
        ## THE STOP IS A LOAD-BEARING RECORD, applied by the SAME proc on
        ## record and on playback (particle-worlds 13c66d7, 2026-08-26). A
        ## wall-clock fact cannot be re-derived from sim state, so writing it
        ## outside `sim.step` while still hashing that tick is exactly what
        ## makes every deadline-ended replay show a hash warning. `applyStop`
        ## banks the battle in progress and finishes the game; the replay
        ## carries one `stop` record and playback calls the identical proc.
        replayWriter.writeChat(
          tickTime(sim.tickCount), 0, stopRecord(sim.tickCount))
    ...
    {.gcsafe.}:
      withLock appState.lock:
        ...
        if not replayLoaded:
          ...
          # record at all (micro round 3, 2026-08-25: "player connected:
          ...
          for websocket, chatText in appState.chatMessages.pairs:
            ...
            if squadMode:
              ...
              # src/smac_starcraft_micro_player.nim), so recording every copy would put
              ...
              if firstRegistration:
                replayWriter.writeChat(
                  ...
                  registerRecord(
                    playerIndex,
                    sim.aliasOfCog(playerIndex),
                    roleText(config.roleOfCog(playerIndex)),
                    policy.label,
                    engine.policyKind(playerIndex),
                    $policy.baseline
                  )
    ...
    # ------------------------------------------------------------------
    #  MICRO: the decision turn, then the control-compiled actuator
    ...
    if squadMode and squadsBuilt and sim.phase == Playing:
      ...
      if sim.gameTicksElapsed() mod turnTicks == 0 and turnKey != lastTurnKey:
        ...
        let records = engine.turn(
          sim, turnIndex, turnsPerBattle, elapsedSeconds)
      ...
      inputs = newSeq[InputState](sim.players.len)
      ## MICRO: only OUR five units get a compiled, recorded mask. Every enemy
      ## decision is re-derived inside `sim.step` from the seed and the state,
      ## so the army costs zero replay bytes.
      let maskCogs =
        if config.microMode(): min(config.numAgents, sim.players.len)
        else: sim.players.len
      for cogIndex in 0 ..< maskCogs:
        ...
        if sim.seatCommands(seat, cogIndex) and seat < engine.directives.len and
          ...
          for candidate in engine.directives[seat].orders:
            if candidate.cogIndex == cogIndex:
              ...
              break
        if not found:
          # The seat has no directive yet — it has not registered, or its first
          # turn has not landed. It plays the published `focusfire` baseline,
          # which is what "adapt to a partner you know the rules of" means
          # here. NO FAILURE MODE LEAVES A UNIT UNACTUATED.
          let scripted = engine.focusfireFor(sim, @[cogIndex])
    ...
    if replayLoaded:
      ...
      for _ in 0 ..< playbackSpeed(liveSpeedIndex):
        ...
        # The micro `fault` end conditions (design §End conditions rows 5
        ...
        try:
          ...
          if not squadMode:
            raise
          echo "micro: SIM GUARD tripped at tick ", sim.tickCount, ": ",
            guard.msg
          faultRule = EndRuleSimFault
        except CatchableError as error:
          if not squadMode:
            raise
          echo "micro: HOST ERROR at tick ", sim.tickCount, ": ",
            error.msg
        ...
        if config.maxGames > 0 and phaseBeforeStep != GameOver and
          ...
          if squadMode:
            if config.microMode():
              ## Archive the battle BEFORE the lobby reset that precedes the
              ...
              echo "battle ", gamesPlayed, " done: ",
                ...
                sim.battleLog[^1].enemyStartHp, ", losses ",
                sim.battleLog[^1].dmgTaken, "/", sim.battleLog[^1].ourStartHp
            else:
              ...
              if config.regimes.len > 0:
                sim.regime =
                  config.regimes[min(gamesPlayed, config.regimes.high)]
    ...
    if quitAfterFrame:
      if squadMode:
        ...
        # table), so a micro replay is self-sufficient — the outcome would
