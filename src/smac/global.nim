...
const
  ...
  ## and is easiest to follow, per ux.replay research): a bright micro HEAD
  ...
  TracerHeadSize = 6           ## the bright leading micro at the impact end.
  ...
  StainObjectBase = 38500      ## one object per stain: 38500..39699, between
                               ...
                               ## swallowed it.
  ## --- Micro FLOOR PAINT (the tile grid) ---
...
proc endzoneStripBox(gameMap: SmacMap, team: Team):
...
proc applyGlobalViewerMessage*(
  ...
  for item in message.parseSpriteClientMessages():
    ...
    of SpriteClientChatMessage:
      # Whole-string micro-side commands are intercepted before the legacy
...
const
  MicroHpBarW* = 34            ## px: the micro hp bar is a CONTINUOUS strip the
                               ## width of a unit's body, not one pip per hit
                               ## point — 60 and 120 hp units would otherwise
                               ## wear a bar wider than the arena.
  MicroHpBarH* = 3             ## px tall, per design §Sim module edit 2.

proc buildMicroHpBar(hp, maxHp: int, ours: bool): seq[uint8] {.measure.} =
  ## MICRO: one unit's overhead health strip, filled to hp / hp_max — WHITE for
  ## our squad, RED for the enemy army. A spectator has to be able to see focus
  ## fire WORKING, not infer it from the feed.
  result = newRgbaPixels(MicroHpBarW, MicroHpBarH)
  let filled = clamp(MicroHpBarW * clamp(hp, 0, max(1, maxHp)) div max(1, maxHp),
                     0, MicroHpBarW)
  for py in 0 ..< MicroHpBarH:
    for px in 0 ..< MicroHpBarW:
      let i = py * MicroHpBarW + px
      if px < filled:
        if ours:
          result.putRawRgbaPixel(i, 236, 240, 244, 240)
        else:
          result.putRawRgbaPixel(i, 224, 82, 58, 240)
      else:
        result.putRawRgbaPixel(i, 30, 27, 24, 175)
...
## Every paint visual below (spray mist, grenade blast, micro tracer + head,
...
proc buildBlastSprite(colorIndex, stage, size: int): seq[uint8] {.measure.} =
  ...
  ## paint-bomb bursts, it doesn't flash white. Same wet-micro language as
...
proc buildTracerDotSprite(colorIndex, stage, bucket: int): seq[uint8] {.measure.} =
  ## Builds one thin trail blob of the comet's tail: a small round wet micro
...
proc buildMuzzleBloomSprite(stage: int): seq[uint8] {.measure.} =
  ...
  ## bright leading micro is the eye-anchor, and the flash must not read as
...
proc buildTracerHeadSprite(colorIndex, stage: int): seq[uint8] {.measure.} =
  ## Builds the bright LEADING micro at a shot's IMPACT end — the comet's
...
proc buildHitSparkSprite(colorIndex, stage: int): seq[uint8] {.measure.} =
  ## Builds the on-hit PAINT SPLAT left by a non-fatal hit (this is micro,
...
proc endzoneShapeToken(gameMap: SmacMap, zone: CaptureZone): string =
...
proc buildFlagBannerSprite(team: Team): seq[uint8] {.measure.} =
  ## The carried objective: a glowing team-colored HEART-GEM relic (Red =
...
proc addShotTracers(
  ...
  ## that fades back toward the shooter, and a bright leading micro at the
...
proc addAimIndicators(
  ...
  ## Aim direction is now shown by the soldier's held micro gun, which
...
proc buildBarrierUpSprite(barrier: PlacedBarrier): (int, int, seq[uint8]) =
  ...
  ## the flat middle side, and one dark dent per micro hit taken.
  let
...
proc addHpPips(
  ...
  for i in 0 ..< sim.players.len:
    ...
    if viewerIndex >= 0 and i != viewerIndex and
        not sim.playerVisibleTo(viewerIndex, i):
      continue
    let micro = sim.config.microMode()
    let maxHp =
      if micro: max(1, player.maxHp)
      else: max(1, sim.config.maxHpFor(player.team, player.perks))
    let hp = clamp(player.hp, 0, maxHp)
    let shieldHp = max(0, player.shieldHp)
    let width = (if micro: MicroHpBarW else: hpBarWidth(maxHp + shieldHp))
    let height = (if micro: MicroHpBarH else: HpBarH)
    ...
    packet.addBoardSpriteChanged(
      ...
      height,
      (if micro:
         buildMicroHpBar(hp, maxHp, not sim.config.isEnemyCog(i))
       else:
    ...
    packet.addBoardObject(
      ...
      player.overheadAnchorY() - OverheadYOffset - height,
      30001,
      MapLayerId,
      spriteId
    )
...
proc buildSpriteProtocolUpdates*(
  ...
  # NEW (micro): the floor-paint grid (incremental, persistent) and the
