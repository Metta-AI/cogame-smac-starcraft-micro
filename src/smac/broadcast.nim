...
type
  BroadcastTracker* = object
    ...
    gameIndex: int
    startedGame: bool
    ## MICRO derived-beat state. All of it is presentation bookkeeping, so it
    ## lives on the tracker (never on the sim) and can never enter gameHash.
    microFirstBlood: bool       ## the `firstblood` beat has already fired.
    microLastCog: bool          ## the `lastcog` beat has already fired.
    microFocusTick: seq[int]    ## per enemy cog: last `focus` beat tick.
    microShots: seq[int]        ## per cog, for the `shot` / `swing` delta.
...
proc snapshot(tracker: var BroadcastTracker, sim: SimServer) =
  ...
  tracker.microShots.setLen(sim.players.len)
  while tracker.microFocusTick.len < sim.players.len:
    tracker.microFocusTick.add(low(int) div 4)
  for i, p in sim.players:
    tracker.microShots[i] = p.shotsFired
  for i, p in sim.players:
...
proc stepEvents*(
  ...
  if sim.phase != tracker.prevPhase:
    ...
    if sim.phase == Playing and sim.config.numAgents > 0 and
        not sim.config.microMode():
      # Paint mode: each GAME of the episode announces itself with its
  ...
  # --- MICRO derived beats ---------------------------------------------------
  # Every one of these is a state DELTA, so they cost no replay bytes and read
  # identically live and in replay. The five scrubber beat kinds this game
  # emits are `battlestart`, `firstblood`, `loss`, `lastcog` and `battleover` —
  # and the appended client block defines CSS for exactly those and no others.
  if sim.config.microMode():
    if sim.phase == Playing and tracker.prevPhase != Playing:
      tracker.microFirstBlood = false
      tracker.microLastCog = false
      events.add(%*{
        "t": tick, "k": "battlestart", "battle": sim.gameIndex + 1,
        "battles": max(1, sim.config.maxGames),
        "scenario": sim.config.scenario,
        "ours": sim.ourAlive, "theirs": sim.theirAlive
      })
    if sim.phase == GameOver and tracker.prevPhase == Playing:
      events.add(%*{
        "t": tick, "k": "battleover", "battle": sim.gameIndex + 1,
        "endRule": (if sim.endRule.len > 0: sim.endRule else: EndRuleFullTime),
        "ticks": sim.gameTicksElapsed(),
        "damagePct": 100 * sim.battleDmgDealt div max(1, sim.enemyStartHp),
        "lossPct": 100 * sim.battleDmgTaken div max(1, sim.ourStartHp)
      })
    for i, p in sim.players:
      if i < tracker.microShots.len and p.shotsFired > tracker.microShots[i]:
        events.add(%*{
          "t": tick, "k": "shot", "by": sim.slotOf(i),
          "byAlias": sim.cogAlias(i), "foe": sim.config.isEnemyCog(i)
        })
      ...
      if i < tracker.hp.len and p.hp < tracker.hp[i]:
        events.add(%*{
          "t": tick, "k": "hit", "target": sim.slotOf(i),
          "targetAlias": sim.cogAlias(i), "foe": sim.config.isEnemyCog(i),
          "dmg": tracker.hp[i] - p.hp, "hpLeft": max(0, p.hp)
        })
      if i < tracker.alive.len and tracker.alive[i] and not p.alive:
        let foe = sim.config.isEnemyCog(i)
        if foe:
          events.add(%*{
            "t": tick, "k": "kill", "alias": sim.cogAlias(i),
            "targetRole": roleText(p.role)
          })
          if not tracker.microFirstBlood:
            tracker.microFirstBlood = true
            events.add(%*{
              "t": tick, "k": "firstblood", "target": sim.cogAlias(i)
            })
        else:
          events.add(%*{
            "t": tick, "k": "loss", "who": sim.slotOf(i),
            "alias": sim.cogAlias(i), "role": roleText(p.role)
          })
      if sim.config.isEnemyCog(i) and i < sim.focusCount.len and
          sim.focusCount[i] >= 2 and i < tracker.microFocusTick.len and
          tick - tracker.microFocusTick[i] >= TargetFps:
        ## Throttled to one per 24 ticks per enemy so a squad holding a target
        ## cannot flood the feed.
        tracker.microFocusTick[i] = tick
        events.add(%*{
          "t": tick, "k": "focus", "target": sim.cogAlias(i),
          "n": sim.focusCount[i]
        })
    if sim.phase == Playing and sim.ourAlive == 1 and not tracker.microLastCog:
      tracker.microLastCog = true
      for i in 0 ..< min(sim.config.friendlyCount(), sim.players.len):
        if sim.players[i].alive:
          events.add(%*{
            "t": tick, "k": "lastcog", "who": sim.slotOf(i),
            "alias": sim.cogAlias(i)
          })
          break

  # NEW (micro): hill ownership changes, banked-second ticks and the
  ...
  # NEW (micro): the three per-cog beats the design note's vocabulary
  ...
  if sim.config.numAgents > 0 and not sim.config.microMode():
    for i, p in sim.players:
      if i < tracker.arcTicks.len and p.arcTicksLeft > 0 and
          tracker.arcTicks[i] == 0:
        events.add(%*{
  ...
  for i, p in sim.players:
    if i < tracker.deaths.len and p.deaths > tracker.deaths[i]:
      ...
      events.add(event)
      if sim.config.numAgents > 0 and not sim.config.microMode():
        # Paint mode: nobody dies there — a cog is TAGGED OUT for two
...
proc teamPoliciesJson(sim: SimServer, team: Team): JsonNode =
  ...
  ## One entry per policy — a mixed team (team-doubles: two policies per side)
...
proc teamStateJson(sim: SimServer, team: Team): JsonNode =
  ...
  if sim.config.hill:
    # --- micro scorebug fields (absent on classic wire frames) ---
  ...
  for i, p in sim.players:
    ...
    if sim.config.numAgents > 0:
      ...
      item["on"] = %(case p.paintUnder
        ...
        of puNone: "none")
    if sim.config.microMode():
      # MICRO roster extras: the unit's role, its hit-point bar, whether it is
      # ours or the scripted army's, its enemy id, its damage ledger and how
      # many of ours are shooting it — everything the scorebug, the hp pips
      # and the focus ring read.
      item["role"] = %roleText(p.role)
      item["hpMax"] = %p.maxHp
      item["foe"] = %sim.config.isEnemyCog(i)
      item["eid"] = %sim.config.enemyIdOf(i)
      item["dmg"] = %p.microDealt
      item["took"] = %p.microTaken
      item["focus"] = %(if i < sim.focusCount.len: sim.focusCount[i] else: 0)
      item["fell"] = %p.deathTick
      item["x"] = %(p.x + CollisionW div 2)
      item["y"] = %(p.y + CollisionH div 2)
      if not sim.config.isEnemyCog(i):
        item["llm"] = %(
          if p.seat >= 0 and p.seat < sim.seatPolicyKind.len:
            sim.seatPolicyKind[p.seat] else: "")
...
proc firstPersonJson(sim: SimServer, playerIndex: int): JsonNode =
  ...
  var ents = newJArray()
  # Micro beams the seat can see (filled below, only while alive — a ghost's
  ...
  if selfAlive:
    ...
    # --- micro beams in flight (sim.recentShots; cosmetic, never hashed) ---
...
proc buildStateJson*(
  ...
  if sim.config.microMode():
    ## MICRO frame extras — the two army bars ARE the score, so the frame
    ## carries both armies' live totals, the battle's starting pools, the
    ## battle ledger and the per-battle log the endcard reads.
    var log = newJArray()
    for battle in sim.battleLog:
      log.add(%*{
        "rule": battle.endRule,
        "ticks": battle.ticks,
        "dmgPct": 100 * battle.dmgDealt div max(1, battle.enemyStartHp),
        "lossPct": 100 * battle.dmgTaken div max(1, battle.ourStartHp),
        "won": battle.won
      })
    state["micro"] = %*{
      "scenario": sim.config.scenario,
      ...
      "battles": max(1, sim.config.maxGames),
      "ourHp": sim.ourHp, "ourMax": max(1, sim.ourStartHp),
      "ourAlive": sim.ourAlive, "ourTotal": sim.config.friendlyCount(),
      "theirHp": sim.theirHp, "theirMax": max(1, sim.enemyStartHp),
      "theirAlive": sim.theirAlive, "theirTotal": sim.config.enemyCount(),
      "dealt": sim.battleDmgDealt, "taken": sim.battleDmgTaken,
      "won": sim.battlesWon,
      "score": microTeamScorePermille(sim),
      "log": log,
      "reason": (if sim.endReason.len > 0: sim.endReason else: ReasonComplete),
      "rule": (if sim.endRule.len > 0: sim.endRule else: "")
    }
