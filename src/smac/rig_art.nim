...
proc loadCrewSpriteRow*(row: int, label: string): seq[CrewSprite] =
  ...
  if row < 0:
    raise newException(SmacError, "Crew sprite sheet row is negative.")
  let
    path = crewSheetPath()
...
proc loadSprayCanSprite*(size: int): seq[uint8] =
  ...
  ## micro — the short-range weapon sprays paint, it does not fire spraypaint).
...
proc ensureGunLoaded() =
  if gunLoaded:
    return
  # Top-down micro marker, muzzle east, barrel on the image mid-line. Scaled
...
proc rigGunPixels*(team: Team, aimStep: int, renderScale = 1): seq[uint8] =
  ## The held top-down micro MARKER as its OWN HUB-centered rig object (not
