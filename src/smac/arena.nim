...
## (selectSmacMap/loadSmacMap — including the import-time default-arena
...
proc validateMapRect(name: string, rect: MapRect, width, height: int) =
  ...
  if rect.w <= 0 or rect.h <= 0:
    raise newException(SmacError, "Map " & name & " size must be positive.")
  if rect.x < 0 or rect.y < 0 or
      rect.x + rect.w > width or rect.y + rect.h > height:
    raise newException(SmacError, "Map " & name & " is outside the map.")
...
proc validateMap(gameMap: SmacMap) =
  ## Raises if a loaded map has invalid geometry.
  if gameMap.width <= 0 or gameMap.height <= 0:
    raise newException(SmacError, "Map dimensions must be positive.")
  case gameMap.layout
  of layoutSides:
    if gameMap.symmetry in {symRot90, symQuadMirror}:
      raise newException(
        SmacError, "Sides maps cannot use a 4-team symmetry (rot90/quadmirror).")
  of layoutCorners, layoutPlus:
    if gameMap.symmetry notin {symRot90, symQuadMirror}:
      raise newException(
        SmacError, "Corner/plus maps need rot90 or quadmirror symmetry.")
...
proc homeDepthOf(gameMap: SmacMap): int =
...
proc rot90Quarter*(gameMap: SmacMap, team: Team): int =
...
proc rot90TeamPoint*(gameMap: SmacMap, red: MapPoint, team: Team): MapPoint =
...
proc teamImagePoint*(gameMap: SmacMap, red: MapPoint, team: Team): MapPoint =
...
proc homeSlot*(gameMap: SmacMap, team: Team): Team =
...
proc slotAnchor*(gameMap: SmacMap, slot: Team): MapPoint =
...
proc teamAnchor*(gameMap: SmacMap, team: Team): MapPoint =
...
proc spawnPocketHalf*(gameMap: SmacMap, team: Team): tuple[w, h: int] =
...
proc plusArmHalf*(gameMap: SmacMap): int =
...
proc plusArmBandOn*(gameMap: SmacMap, span: int): tuple[lo, hi: int] =
...
proc defaultSmacRooms(gameMap: SmacMap): seq[Room] =
...
proc arenaLargeSmacMap(): SmacMap =
...
proc captureZone*(gameMap: SmacMap, team: Team): CaptureZone =
...
proc `==`*(a, b: ArenaShape): bool =
  ...
  ## SmacMap values compare, which the map-spec round-trip tests rely on.
...
proc symmetryImages*(gameMap: SmacMap, rect: MapRect): seq[MapRect] =
...
proc symmetryImages*(gameMap: SmacMap, point: MapPoint): seq[MapPoint] =
...
proc buildArenaObstacles*(gameMap: SmacMap): seq[ArenaShape] =
...
proc isSpinningDiamond*(gameMap: SmacMap, shape: ArenaShape): bool {.inline.} =
...
proc buildAnimatedDiamonds*(
  gameMap: SmacMap, obstacles: seq[ArenaShape]
...
proc centerPuddleSplat(rng: var MapRng, gameMap: SmacMap): Puddle =
...
proc mapSizeScale(sizeName: string): float =
  ...
  of "colossal": 5.2  ## override-only (not in MapSizeNames): 2x giant.
    ...
    raise newException(SmacError, "Unknown map size: " & sizeName)

proc scaledGenShell(sizeName: string): SmacMap =
...
proc mapProtectedFloorAt*(gameMap: SmacMap, x, y: int): bool =
...
proc validateMapWalkability*(gameMap: SmacMap) =
...
proc scaledGenShell4(sizeName: string): SmacMap =
...
proc sightlineLoX*(gameMap: SmacMap): int =
...
proc sightlineHiX*(gameMap: SmacMap): int =
...
proc sightlineLoY*(gameMap: SmacMap): int =
...
proc sightlineHiY*(gameMap: SmacMap): int =
...
proc rectOnOpenFloor(
  gameMap: SmacMap, obstacles: seq[ArenaShape], rect: MapRect
): bool =
...
proc placePuddles(gameMap: var SmacMap, count: int, rng: var MapRng) =
  ...
  for room in gameMap.defaultSmacRooms():
    if room.name != "Center":
  ...
  proc addPuddlePair(
    gameMap: SmacMap, splats: var seq[Puddle], splat: Puddle
  ): bool =
...
proc placePuddles*(gameMap: var SmacMap, count: int, seed: int) =
  ...
  if count > MaxPuddles:
    raise newException(
      SmacError, "Config field mapPuddles must be 0.." & $MaxPuddles & ".")
  if count > 0 and gameMap.symmetry in {symRot90, symQuadMirror}:
    raise newException(
      SmacError, "Puddles are not supported on 4-team maps yet.")
  gameMap.puddles = @[]
...
proc generateMapAttempt*(
...
): SmacMap =  ## One UNVALIDATED draw. Every top-level parameter is drawn unconditionally
  ...
  if teams == 4:
    ...
    if overrides.symmetry notin ["", "rot90", "quadmirror"]:
      raise newException(
        SmacError, "4-team maps are rot90 or quadmirror; got mapSymmetry: " &
          overrides.symmetry)
    ...
    result.layout =
      ...
      of "plus": layoutPlus
        ...
        raise newException(
          SmacError, "Unknown map layout: " & overrides.layout)
  else:
    ...
    result.symmetry =
      ...
      of "rot180": symRot180
        ...
        raise newException(
          SmacError, "Unknown map symmetry: " & overrides.symmetry)
    if overrides.layout.len > 0 and overrides.layout != "sides":
      raise newException(
        SmacError, "Map layout " & overrides.layout & " needs teams: 4.")
  ...
  block endzoneDraw:
    ...
    result.endzone =
      ...
      of "square": ezSquare
        ...
        raise newException(
          SmacError, "Unknown map endzone: " & overrides.endzone)
    ...
    if teams == 4:
      raise newException(
        SmacError, "Compact endzones (mapEndzone) need a 2-team map.")
    ...
    if result.homeDepth < HomeDepthMin or result.homeDepth > HomeDepthMax:
      raise newException(
        SmacError, "Config field mapBaseDepth must be " & $HomeDepthMin &
          ".." & $HomeDepthMax & ".")
    if result.endzoneRadius < EndzoneRadiusMin or
        result.endzoneRadius > maxEndzoneRadius(result.width):
      raise newException(
        SmacError, "Config field mapEndzoneRadius must be " &
  ...
  if feature notin CenterFeatureNames:
    raise newException(SmacError, "Unknown map center feature: " & feature)
  ...
  if columns < 3 or columns > maxColumns:
    raise newException(
      SmacError, "Config field mapColumns must be 3.." & $maxColumns & ".")
  ...
  if overrides.pits < -1 or overrides.pits > 64:
    raise newException(SmacError, "Config field mapPits must be 0..64.")
  if overrides.puddles > MaxPuddles:
    raise newException(
      SmacError, "Config field mapPuddles must be 0.." & $MaxPuddles & ".")
  if overrides.pitDensity < -1 or overrides.pitDensity > 1000:
    raise newException(
      SmacError, "Config field mapPitDensity must be 0..1000.")
  let
  ...
  if result.symmetry in {symRot90, symQuadMirror}:
    ...
    if overrides.pits > 0:
      raise newException(
        SmacError, "Trenches are not supported on 4-team maps yet.")
  ...
  block sightlineRepair:
    proc rowBlocked(gameMap: SmacMap, y: int): bool =
      for x in gameMap.sightlineLoX .. gameMap.center.x:
        if mapWallAt(gameMap, gameMap.leftObstacles, x, y):
          return true
      false
    proc rowBlockedFull(gameMap: SmacMap, obstacles: seq[ArenaShape],
        y: int): bool =
  ...
  block columnSightlineRepair:
    ...
    proc colBlocked(gameMap: SmacMap, x: int): bool =
      ...
      for y in gameMap.sightlineLoY .. gameMap.sightlineHiY:
        if mapWallAt(gameMap, gameMap.leftObstacles, x, y,
          ...
          return true
      false
    proc colFullyProtected(gameMap: SmacMap, x: int): bool =
  ...
  if windowCount > 6:
    raise newException(SmacError, "Config field mapWindows must be 0..6.")
  ...
  block finalizeTrenches:
    ...
    if oddCenterPit:
      ...
      digs.add centerPit
    proc addPair(
      gameMap: SmacMap, digs: var seq[MapRect], trench: MapRect
    ): bool =
...
proc validateGeneratedMap*(gameMap: SmacMap): string =
...
proc poolSmacMap*(
  ...
  generateSmacMap(MapPoolSeeds[((index mod n) + n) mod n], overrides)
...
proc shapeFromSpecNode(node: JsonNode): ArenaShape =
  ...
  of "polygon":
    ...
    raise newException(
      SmacError, "Unknown map spec shape: " & node["kind"].getStr())
...
proc mapSpecJson*(gameMap: SmacMap): string =
...
proc mapFromSpecJson*(text: string): SmacMap =
  ...
  try:
    ...
    raise newException(SmacError, "Could not parse map spec JSON: " & e.msg)
  ...
  result.symmetry =
    ...
    of "none": symNone          # coworld-ctf#280 full-board authoring (no lift)
      ...
      raise newException(
        SmacError, "Unknown map spec symmetry: " & symmetryText)
  ...
  result.layout =
    ...
    of "plus": layoutPlus
      ...
      raise newException(SmacError, "Unknown map spec layout: " & layoutText)
  ...
  result.endzone =
    ...
    of "square": ezSquare
      ...
      raise newException(SmacError, "Unknown map spec endzone: " & endzoneText)
...
proc resolveSmacMapMetadata*(config: GameConfig): SmacMap =
  ...
  result =
    if config.mapSpec.len > 0:
      ...
      of PoolMapName:
        if config.teams != 2:
          raise newException(
            SmacError, "The curated pool is 2-team; use mapPath gen for " &
              $config.teams & " teams.")
        let index =
          if config.mapPoolIndex >= 0: config.mapPoolIndex else: genSeed
        poolSmacMap(index, config.mapGen)
      else:
        raise newException(SmacError, "Unknown map: " & name)
  if result.teamCount() != config.teams:
    raise newException(
      SmacError, "Config asks for " & $config.teams & " teams but map " &
...
## The SELECTED map's layout, installed once per process by loadSmacMap and
...
proc selectSmacMap(gameMap: SmacMap) =
...
proc installDefaultArena*() =
  ...
  ## import-time `selectSmacMap(arenaSmacMap())` side effect with this explicit
  ## call: constructing any sim (initSimServer -> loadSmacMap) installs its
  ...
  selectSmacMap(arenaSmacMap())

proc loadSmacMapMetadata*(path = ""): SmacMap =
  ...
  of ArenaLargeName: arenaLargeSmacMap()
    ...
    if parts.len <= 2 and parts[0] == GenMapName:
      generateSmacMap(suffix)
    elif parts.len <= 2 and parts[0] == PoolMapName:
      poolSmacMap(suffix)
    else:
      raise newException(SmacError, "Unknown map: " & name)

proc loadSmacMapMetadata*(config: GameConfig): SmacMap =
  ## GameConfig-driven metadata: honors mapSpec, mapSeed, pool picks, and
  ## the generator overrides.
  resolveSmacMapMetadata(config)

proc loadSmacMap*(path = ""): SmacMap =
  ...
  result = loadSmacMapMetadata(path)
  selectSmacMap(result)

proc loadSmacMap*(config: GameConfig): SmacMap =
  ## Resolves the config's effective map and installs it as this process's
  ## arena.
  result = resolveSmacMapMetadata(config)
  selectSmacMap(result)
...
proc arenaCenterOffset2(x, y, cx, cy: int): tuple[dx, dy: int] {.inline.} =
  ## The installed-map twin of SmacMap.centerOffset2: twice the offset from
...
proc mapProtectedFloorAtF*(
  gameMap: SmacMap, x, y: float, cx, cy: int
): bool =
