...
proc spawnAimBrads*(gameMap: SmacMap, team: Team): int =
...
proc spawnFlipH*(gameMap: SmacMap, team: Team): bool =
...
proc gameHash*(sim: SimServer): uint64 =
  ...
  if sim.config.numAgents == 0:
    ## In a seat-commanded (micro) episode `needsReregister` is live-server
  ...
  # --- micro state, APPENDED after every existing mix so the ordering of
  ...
  # --- MICRO state, APPENDED after every existing mix so the ordering of the
  # inherited fields stays stable. All of it is gameplay state the wasm viewer
  # re-derives from the recorded friendly masks — including the WHOLE enemy
  # army, which is never recorded — so all of it is hashed. `focusCount` is
  # DERIVED from these and is deliberately left out.
  if sim.config.loadout == LoadoutMicro:
    for player in sim.players:
      result.mixHashInt(ord(player.role))
      result.mixHashInt(player.maxHp)
      result.mixHashInt(player.speedPct)
      result.mixHashInt(player.enemyId)
      result.mixHashInt(player.microDealt)
      result.mixHashInt(player.microTaken)
      result.mixHashInt(player.targetSeat)
      result.mixHashInt(player.retargetCounter)
      result.mixHashInt(player.stuckCount)
      result.mixHashInt(player.stuckRotTicks)
      result.mixHashInt(player.lastPosX)
      result.mixHashInt(player.lastPosY)
      result.mixHashBool(player.heldAttack)
    result.mixHashInt(sim.ourHp)
    result.mixHashInt(sim.theirHp)
    result.mixHashInt(sim.ourAlive)
    result.mixHashInt(sim.theirAlive)
    result.mixHashInt(sim.ourStartHp)
    result.mixHashInt(sim.enemyStartHp)
    result.mixHashInt(sim.battleDmgDealt)
    result.mixHashInt(sim.battleDmgTaken)
    result.mixHashInt(sim.battlesWon)
    result.mixHashInt(sim.battleIndex)
