...
const
  GameName* = "smac-starcraft-micro"
  GameVersion* = "1"  ## GV1 (smac-starcraft-micro): the first version of this
    ## coworld, forked from coworld-ctf GV44. The micro loadout adds the
    ## per-role unit table, the in-sim scripted enemy army, the battle
    ## bookkeeping and the load-bearing wall-clock `stop` record; the
    ## inherited combat core, replay codec and hash chain are unchanged.
    ## Previously (coworld-ctf) GV44 (seating rule): THE HOMES ARE DEALT, NOT OWNED.
  ...
  ## micro gun master (paintgun.png, muzzle east). Body and gun are mounted
  ...
  BarrierHp* = 10             ## micro hits a placed barrier soaks before
  ...
  # --- Micro King of the Hill (docs/plans/2026-08-25-micro-design.md) ---
  # Every value below is a DEFAULT for the matching GameConfig field; a
  # variant may override it. All micro arithmetic is integer-only so the
  ...
  DefaultSprayDamage* = 1       ## hp per cone touch under the micro loadout
  ...
  ## seconds — so attempt 1 really ran with 4 s. Micro's own sidecar
  ...
  LoadoutClassic* = "ctf"       ## the starter's loadout: pickups, gun, hearts.
  LoadoutPaint* = "paint"       ## the starter's paint loadout (kept, unused).
  LoadoutMicro* = "micro"       ## THIS game: two armies, no pickups, no hearts,
                                ## no respawn, per-role weapons.
  ...
  # --- smac micro defaults (docs/RULES.md). ------------------------------
  # Every one of these is a config field with this value as its default, so a
  # variant that omits it plays the published rules.
  DefaultScenario* = "default"
  DefaultMaxGamesMicro* = 3
  DefaultMaxTicksMicro* = 1440   ## 60 s of battle at 24 ticks/s.
  DefaultTurnTicksMicro* = 120   ## 5.0 s per decision turn -> 12 turns/battle.
  DefaultFriendlySpawnX* = 380
  DefaultEnemySpawnX* = 855
  DefaultSpawnSpacingPx* = 44
  DefaultSpawnJitterPx* = 24
  DefaultRangerHp* = 60
  DefaultRangerRange* = 380
  DefaultRangerDamage* = 4
  DefaultRangerCooldown* = 18
  DefaultBladeHp* = 120
  DefaultBladeSpeedPct* = 115
  DefaultBladeReach* = 56
  DefaultBladeArcBrads* = 32     ## half-angle in brads: 32 = 45 degrees.
  DefaultBladeDamage* = 10
  DefaultBladeCooldown* = 30
  DefaultSwingTicks* = 4         ## ticks a thrown swing stays lit.
  DefaultSwarmHp* = 30
  DefaultSwarmSpeedPct* = 130
  DefaultSwarmReach* = 40
  DefaultSwarmDamage* = 6
  DefaultSwarmCooldown* = 24
  DefaultAggroPx* = 600
  DefaultLeashPx* = 700
  DefaultRetargetTicks* = 48
  DefaultEnemyStuckTicks* = 24
  DefaultEnemyStuckRotTicks* = 12
  DefaultRangerStandoff* = 300
  DefaultKiteStandoff* = 340
  DefaultScreenStandoff* = 90
  DefaultChaseCapPx* = 520
  DefaultPanicPx* = 150
  DefaultWinWeightPermille* = 600
  DefaultDmgWeightPermille* = 300
  DefaultSurvWeightPermille* = 100
  DefaultCreditEpsilonPerMyriad* = 4
  MaxEnemyUnits* = 24            ## config bound on enemyRoles.
  MaxEnemyViewEntries* = 24      ## cap on the seat view's `enemies` array.
  EndRuleVictory* = "victory"
...
## process by loadSmacMap (driven by config.mapPath) BEFORE any sim, mask,
...
## initialized to the default arena so tools that never call loadSmacMap
...
type
  UnitRole* = enum
    ## What a unit is. `urSwarm` is enemy-only (the corridor scenario); ours
    ## are always `urRanger` or `urBlade`, one per seat.
    urRanger = "ranger"
    urBlade = "blade"
    urSwarm = "swarm"

  BattleRecord* = object
    ## One finished battle of the episode, archived by the server before the
    ## lobby reset that precedes the next battle.
    ticks*: int
    endRule*: string
    dmgDealt*: int
    dmgTaken*: int
  ...
  SmacError* = object of ValueError
  ...
  SmacMap* = object
    ...
    homeRotation*: int         ## GV44: per-episode rotation of TEAM -> HOME
                               ...
                               ## `resolveSmacMapMetadata`, so it is a pure
  ...
  MapGenOverrides* = object
    ...
    baseDepth*: int        ## home anchor depth permille (see SmacMap.
  ...
  GameConfig* = object
    ...
    # --- micro gates (all OFF by default: a gate-off config plays the
    ...
    loadout*: string              ## LoadoutClassic (default) or LoadoutPaint.
    ...
    # --- smac micro gates (appended; a config that leaves `loadout` at
    # LoadoutClassic plays the inherited game byte-for-byte) ---
    scenario*: string             ## the shipped variant id, echoed into results.
    fogOfWar*: bool               ## false in every shipped micro variant: the
                                  ## board is fully observable (design
                                  ## §Per-seat observation).
    roles*: seq[UnitRole]         ## our five units, seat order.
    enemyRoles*: seq[UnitRole]    ## the enemy army, 1..MaxEnemyUnits.
    friendlySpawnX*: int
    enemySpawnX*: int
    spawnSpacingPx*: int
    spawnJitterPx*: int
    ...
    chaseCapPx*: int
    panicPx*: int
    winWeightPermille*: int
    dmgWeightPermille*: int
    survWeightPermille*: int
    creditEpsilonPerMyriad*: int
  ...
  Player* = object
    ...
    grenadeDamageDealt*: int   ## the grenade-blast share of damageDealt;
                               ## analysis-only, excluded from gameHash.
    gunDamageDealt*: int       ## the micro-gun share of damageDealt
    ...
    # --- smac micro state (appended; keyframes are flatty-POSITIONAL, so
    # new fields may only be appended). All of it is HASHED except deathTick.
    role*: UnitRole            ## what this unit is.
    maxHp*: int                ## the role's starting hit points.
    speedPct*: int             ## the role's speed, percent of config.maxSpeed.
    enemyId*: int              ## 0 = one of ours; 1..n = the enemy army's id,
                               ## assigned in spawn order and stable for the
                               ## battle. This is the id a commander names in
                               ## `target_id`.
    targetSeat*: int           ## enemy AI: the friendly cog it is closing on,
                               ## -1 = none. Meaningless for our units.
    retargetCounter*: int      ## enemy AI: ticks until the forced re-scan.
    stuckCount*: int           ## enemy AI: consecutive motionless ticks.
    stuckRotTicks*: int        ## enemy AI: remaining quarter-turn deflection.
    microDealt*: int           ## damage this unit dealt to the OTHER side,
                               ## overkill clipped. HASHED.
    microTaken*: int           ## damage this unit absorbed, overkill clipped.
                               ## HASHED.
    deathTick*: int            ## the tick this unit fell, -1 = still standing.
                               ## Presentation only, never hashed.
    lastPosX*, lastPosY*: int  ## enemy AI: position at the previous tick, the
                               ## input to the stuck detector. HASHED.
    heldAttack*: bool          ## enemy AI: whether the trigger was held last
                               ## tick, so a FRESH press can be derived without
                               ## a recorded mask. HASHED.
  ...
  PlacedBarrier* = object
    ...
    hp*: int                   ## micro hits left (starts at BarrierHp).
  ...
  SimServer* = object
    ...
    placedBarriers*: seq[PlacedBarrier]  ## standing cardboard barriers,
                               ...
                               ## pre-barrier builds.
    # --- micro state (appended at the END of the type: keyframes are
    ...
    llmTurns*: seq[int]        ## per seat: turns whose directive came from an LLM.
    fallbackTurns*: seq[int]   ## per seat: turns that fell back to scripted.
    ...
    # --- smac micro state (appended at the END of the type) ---
    ourHp*, theirHp*: int          ## live hit-point totals, both armies.
    ourAlive*, theirAlive*: int    ## live unit counts, both armies.
    ourStartHp*, enemyStartHp*: int ## the battle's starting pools.
    battleDmgDealt*, battleDmgTaken*: int  ## this battle, overkill clipped.
    battlesWon*: int               ## victories banked this episode.
    battleIndex*: int              ## 0-based index of the battle in play.
    battleLog*: seq[BattleRecord]  ## one entry per battle finished.
    focusCount*: seq[int]          ## per cog: how many of ours are currently
                                   ## attacking it. DERIVED, never hashed.
    lastFocusTick*: seq[int]       ## per cog: throttle for the `focus` beat.
    seatDamage*: seq[int]          ## per seat: clipped damage dealt, EPISODE.
    seatTaken*: seq[int]           ## per seat: clipped damage taken, episode.
    seatKills*: seq[int]           ## per seat: enemy units killed, episode.
    seatDeaths*: seq[int]          ## per seat: battles this seat died in.
    seatShots*: seq[int]           ## per seat: attacks released, episode.
...
proc teamCount*(gameMap: SmacMap): int =
...
proc teams*(gameMap: SmacMap): Slice[Team] =
...
proc parsePerk*(text: string): Perk =
  ## Parses one authored perk name; raises SmacError on an unknown name.
  for perk in Perk:
    if PerkNames[perk] == text:
      return perk
  raise newException(SmacError, "Unknown perk name: " & text)
...
proc roleText*(role: UnitRole): string {.inline.} =
  ## The authored/wire name of one unit role.
  case role
  of urRanger: "ranger"
  of urBlade: "blade"
  of urSwarm: "swarm"

proc parseUnitRole*(text: string): UnitRole =
  ## Parses one authored role name; raises SmacError on an unknown name.
  case text.strip().toLowerAscii()
  of "ranger": urRanger
  of "blade": urBlade
  of "swarm": urSwarm
  else: raise newException(SmacError, "Unknown unit role: " & text)

proc roleHp*(config: GameConfig, role: UnitRole): int {.inline.} =
  ## The role's starting hit points.
  case role
  of urRanger: max(1, config.rangerHp)
  of urBlade: max(1, config.bladeHp)
  of urSwarm: max(1, config.swarmHp)

proc roleSpeedPct*(config: GameConfig, role: UnitRole): int {.inline.} =
  ## The role's speed as a percent of config.maxSpeed.
  case role
  of urRanger: 100
  of urBlade: max(1, config.bladeSpeedPct)
  of urSwarm: max(1, config.swarmSpeedPct)

proc roleDamage*(config: GameConfig, role: UnitRole): int {.inline.} =
  ## Hit points one landed attack of this role removes.
  case role
  of urRanger: max(1, config.rangerDamage)
  of urBlade: max(1, config.bladeDamage)
  of urSwarm: max(1, config.swarmDamage)

proc roleCooldown*(config: GameConfig, role: UnitRole): int {.inline.} =
  ## Ticks between two attacks of this role.
  case role
  of urRanger: max(1, config.rangerCooldown)
  of urBlade: max(1, config.bladeCooldown)
  of urSwarm: max(1, config.swarmCooldown)

proc roleReach*(config: GameConfig, role: UnitRole): int {.inline.} =
  ## The role's weapon reach in map pixels: a ranger's shot range, a melee
  ## unit's swing reach.
  case role
  of urRanger: max(1, config.rangerRange)
  of urBlade: max(1, config.bladeReach)
  of urSwarm: max(1, config.swarmReach)

proc isMelee*(role: UnitRole): bool {.inline.} =
  ## True for the two arc-cone roles.
  role in {urBlade, urSwarm}
