...
proc randomSeed(): int =
  ...
  if not urandom(buf):
    raise newException(SmacError, "OS entropy source unavailable")
...
proc echoStartupConfig(
  ...
  echo "SMAC config: host=", runtimeConfig.host,
...
when isMainModule:
  let
    ...
    localReplayPath =
      if runtimeConfig.replayUri.len > 0:
        getTempDir() / ("smac-replay-" & $getCurrentProcessId() &
          ".bitreplay")
      else:
        ""
  ...
  let loadReplayPath =
    if runtimeConfig.replayMode:
      let path = getTempDir() / ("smac-load-replay-" &
  ...
  echo "starting smac-starcraft-micro on ", runtimeConfig.host, ":", runtimeConfig.port
  runServerLoop(
    runtimeConfig.host,
    runtimeConfig.port,
    config,
