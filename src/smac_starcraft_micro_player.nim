## The smac-starcraft-micro player container: a policy is just a prompt.
##
...
##   PLAYER_SCRIPTED      focusfire | charge         -> this seat is scripted
##   PLAYER_POLICY_LABEL  a free label for the replay's `register` record
##
## A seat that sets neither is `focusfire`. To field your own policy, reuse
## this image and set PLAYER_PROMPT:
##
##   coworld upload-policy coworld-smac-starcraft-micro --name my-smac \
...
when isMainModule:
  ...
  echo "smac player: kind=",
    (if prompt.len > 0: "llm" else: "scripted"),
    " baseline=", (if scripted.len > 0: scripted else: "focusfire"),
    " label=", label
  ...
  proc dial(attempts: int): WebSocket =
    ...
    for attempt in 0 ..< attempts:
      try:
        ...
        if attempt == 0:
          echo "smac player: game not listening yet (", error.msg,
            "); retrying"
  ...
  if socket == nil:
    quit("smac player: game never accepted a connection", 1)
  echo "smac player: connected"
  ...
  # episode (micro round 3, 2026-08-25). The server now holds an
  ...
  while true:
    ...
    try:
      ...
      echo "smac player: socket closed (", error.msg, ")"
    ...
    if sessionFrames == 0 or reconnects >= ReconnectAttempts:
      break
    inc reconnects
    echo "smac player: re-dialling the seat (attempt ", reconnects, ")"
    socket = dial(ReconnectAttempts)
    if socket == nil:
      echo "smac player: game is no longer listening, exiting cleanly"
      break
    echo "smac player: reconnected, re-registering"
  quit(0)
