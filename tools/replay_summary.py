#!/usr/bin/env python3
"""Summarise a smac-starcraft-micro `.replay` as one strict-UTF-8 JSON object.

Python 3 standard library only: no Nim, no Docker, no emsdk. This is the JSON
view of the binary `COWLDSMC` replay the static wasm viewer parses, and it is
...
bytes:
    ...
    jq -r '.protocol, .results.reason, .results.enemyKilled' /tmp/ep.json
...
* the CONTROL records - `register`, `directive`, `fallback`,
  `budget_guard`, `stop`, `result` - are UTF-8 JSON objects embedded verbatim in the
...
def summarise(path: str) -> dict:
    ...
    try:
        ...
        # Split AFTER the magic, so the digit scan runs from the
        # gameName + gameVersion region.
        if "COWLDSMC" in head_text:
            head_text = head_text.split("COWLDSMC", 1)[1]
        if "smac-starcraft-micro" in head_text:
            tail = head_text.split("smac-starcraft-micro", 1)[1]
            digits = ""
            for ch in tail:
                if ch.isdigit():
                    digits += ch
    ...
    enemy_roles = list(config.get("enemyRoles") or [])

    # The in-game aliases, derived from the config exactly the way the sim
    # derives them: "<ROLE>-<identity>" ranked among the seats that share the
    # role for ours, "E<n>" for the enemy army.
    identities = ("alpha", "beta", "gamma", "delta", "epsilon",
                  "zeta", "eta", "theta")
    aliases: list[str] = []
    seen: dict[str, int] = {}
    for role in roles:
    ...
    aliases += [f"E{i + 1}" for i in range(len(enemy_roles))]
    ...
    return {
        ...
        "battles": int(config.get("maxGames") or 0),
        "tickCount": len(data),
