...
##   header: "SMCFRM01" | u16 slots | u16 mapW | u16 mapH | u16 teams
...
# `key`/`jsonRow` moved to `smac/events` so the live server and this tool share
...
const
  UsageText =
    "Usage: nim r tools/extract_events.nim [replay-path] [--out <path>] " &
      "[--frames <path>]"
  FramesMagic = "SMCFRM01"
  SeatRecordBytes = 10
  DefaultReplayPath = GameDir / "tests" / "replays" / "episode.bitreplay"
